 ...
 *
 * Every alert on this channel — delay, gate change, cancellation, rebooking — plays the cabin
 * "fasten seatbelt" chime (`res/raw/cabin_chime.wav`) instead of the default notification sound.
 */
object JetNotifier {

    // _v2: a channel's sound is locked in at creation, so shipping the cabin chime required a new
    // channel id (the old default-sound channel is deleted in ensureChannel).
    private const val CHANNEL_ID = "disruption_alerts_v2"
    private const val LEGACY_CHANNEL_ID = "disruption_alerts"
    ...
    private fun ensureChannel(context: Context) {
        ...
        // Retire the pre-chime channel so installs that created it don't keep the default sound.
        ...
        manager.createNotificationChannel(
            NotificationChannel(
                ...
                description = "Flight delays, gate changes, cancellations, and rebooking updates"
                setSound(
                    chimeUri(context),
    ...
    /** The bundled cabin "fasten seatbelt" chime, addressed as a resource URI. */
    private fun chimeUri(context: Context): Uri =
        Uri.parse("android.resource://${context.packageName}/${R.raw.cabin_chime}")
}
