...
> - [`docs/DESIGN_SYSTEM.md`](docs/DESIGN_SYSTEM.md) — full color/typography/spacing tokens (`JetTheme` accessor; `JetSetterTheme { }` wrapper), `JetCard`, `AccentTag`.
...
> - [`docs/IOS_PARITY_NOTES.md`](docs/IOS_PARITY_NOTES.md) — cross-platform contracts (Firestore schema, anonymous auth, IRIS provider) the iOS app must match to stay identical.
...
| **IRIS** | `IRISChatView` | `feature.iris.IrisChatScreen` | ✅ **Wired** — live Gemini via Firebase AI Logic + demo fallback; streaming pending (§8) |
...
> **IRIS** = *Intelligent Routing & Itinerary Specialist* — the in-app assistant. On iOS she ran on Apple Intelligence (on-device) with a Claude fallback. **On Android, IRIS runs on Gemini via Firebase AI Logic** (see [`docs/IOS_PARITY_NOTES.md`](docs/IOS_PARITY_NOTES.md) for the cross-platform contract). See §8.
...
- Decorative gradients/cards are reproduced with Compose `Brush`/`drawBehind`, matching `JetTheme` (§5, and `docs/DESIGN_SYSTEM.md`).
...
├── docs/
│   ├── DESIGN_SYSTEM.md                  ← colors, type, spacing, JetCard/AccentTag
...
└── app/
    ...
    └── src/main/
        ...
        │   ├── ui/
        │   │   ├── theme/                 ← Color.kt, Type.kt, Theme.kt (JetSetterTheme wrapper + JetTheme accessor), Spacing.kt
        │   │   ├── components/            ← reusable Composables (JetCard, AccentTag, ScaffoldNotice, PremiumTextField…)
        │   │   └── navigation/            ← JetNavHost, Destinations, JetBottomBar
        │   ├── feature/
        │   │   ├── home/                  ← HomeScreen, HomeViewModel, HomeUiState
        │   │   ├── itinerary/             ← ItineraryScreen, ItineraryViewModel, AddItem…
        │   │   ├── iris/                  ← IrisChatScreen, IrisChatViewModel, ChatMessage
...
Text(
    ...
    modifier = Modifier
        ...
        .background(JetTheme.colors.warning.copy(alpha = 0.15f), CircleShape)
...
fun JetNavHost(navController: NavHostController, modifier: Modifier = Modifier) {
    NavHost(navController, startDestination = Dest.Home.route, modifier = modifier) {
        ...
        composable(Dest.Iris.route)      { IrisChatScreen() }
...
The design system is ported in `ui/theme/`: use the `JetTheme` accessor object (`JetTheme.colors/typography/spacing`, mirroring `MaterialTheme`) and wrap content in the `JetSetterTheme { }` composable. The colors come straight from the iOS `JetsetterTheme` — note the brand is **deep navy + sky blue** (the iOS gradient is *named* `goldGradient` for historical reasons but renders **blue**). On Android the historical "gold" naming was **dropped**: the accent capsule chip is `AccentTag` (not `GoldTag`). See `docs/DESIGN_SYSTEM.md`.
...
fun ThemedExample() {
    JetCard {                                   // ≈ .jetCard() — glass/elevated surface
        Column {
            Text("Tokyo Spring Trip", color = JetTheme.colors.textPrimary)
            AccentTag(text = "On Time", icon = Icons.Filled.CheckCircle)   // accent capsule chip
        }
    }
}
```
...
## 8. The IRIS Assistant (Firebase AI Logic · Gemini)

**IRIS** (*Intelligent Routing & Itinerary Specialist*) is the conversational assistant on the IRIS tab. On iOS, `AIService` routed: **Apple Intelligence on-device → Claude fallback → mock**. **On Android, IRIS runs on [Firebase AI Logic](https://firebase.google.com/docs/ai-logic) (Gemini)** via the no-cost *Google AI* backend, keeping a canned demo tier so the tab always responds.

> **🔁 iOS parity:** the two platforms intentionally use **different AI providers** (iOS = Apple Intelligence/Claude; Android = Gemini via Firebase AI Logic). Keep the **system prompt, tone, and demo replies identical** so IRIS *behaves* the same on both — only the backend differs. Making the providers themselves match is a product decision, not a port detail. See [`docs/IOS_PARITY_NOTES.md`](docs/IOS_PARITY_NOTES.md).

### Provider routing (Android)

```
Firebase AI call succeeds  ──▶  Gemini reply
Firebase AI call fails     ──▶  Demo reply (inline in IrisRepository) ← always works, even if AI Logic is off/offline
```

### Setup

1. Dependency (BoM-managed, versionless): `implementation("com.google.firebase:firebase-ai")` in `app/build.gradle.kts`.
2. **Enable Firebase AI Logic** for the project once: Firebase console → **AI Logic → Get started** (the *Gemini Developer API* backend is the free tier — no Blaze plan). Until this is done, live calls fail and IRIS uses demo replies.
3. The model is provided by Hilt in `core.di.FirebaseModule`, with the model id and system prompt in one place:

```kotlin
@Provides @Singleton
fun provideIrisModel(): GenerativeModel =
    Firebase.ai(backend = GenerativeBackend.googleAI())          // free tier; vertexAI() needs Blaze
        .generativeModel(
            modelName = "gemini-2.5-flash",                      // one constant, easy to bump
            systemInstruction = content { text(IRIS_SYSTEM_PROMPT) },
        )
```

> **Auth note:** there is **no app-side API key**. Firebase AI Logic provisions and uses the Gemini key server-side; the SDK authenticates via the Firebase app (set up **App Check** before production). Nothing IRIS-related lives in `local.properties` anymore.

### Chat turns

IRIS history is a list of provider-neutral turns (`core.ai.AiMessage`), decoupled from the UI's `ChatMessage` and from any vendor DTO. Gemini roles are **`"user"`** and **`"model"`** (model = assistant):

```kotlin
data class AiMessage(val role: String, val text: String)   // role: "user" | "model"
```
...
class IrisRepository @Inject constructor(
    private val model: GenerativeModel,                  // provided by FirebaseModule
) {
    /** [history] is the full conversation; the last entry is the new user message. */
    suspend fun ask(history: List<AiMessage>): String {
        val last = history.lastOrNull() ?: return demoResponse("")
        // Gemini requires history to START with a user turn — drop IRIS's opening greeting.
        ...
        }.getOrElse { demoResponse(last.text) }          // AI off / offline → canned reply
    }

    private fun demoResponse(prompt: String): String = /* keyword-matched, on-brand replies */ TODO()
}
```
...
class IrisChatViewModel @Inject constructor(
    ...
    private val _messages = MutableStateFlow(listOf(/* opening greeting, fromUser = false */))
    val messages: StateFlow<List<ChatMessage>> = _messages.asStateFlow()
    private val _isThinking = MutableStateFlow(false)
    val isThinking: StateFlow<Boolean> = _isThinking.asStateFlow()

    fun send(text: String) {
        // append the user turn + set isThinking = true, then:
        viewModelScope.launch {
            val aiHistory = _messages.value.map { AiMessage(if (it.fromUser) "user" else "model", it.text) }
...
`IrisChatScreen` collects `messages` and `isThinking` with `collectAsStateWithLifecycle()` and renders a message list + input bar, showing a typing indicator while `isThinking`.

### Where the demo / canned fallback lives

The fallback lives **inline in `IrisRepository.demoResponse(...)`** (keyword-matched, on-brand answers, ported from iOS `IRISDemoResponses.swift`). It is the offline / not-yet-enabled safety net — **the IRIS tab must always respond**, exactly as on the iOS simulator.

> **Streaming** (token-by-token) is the next enhancement: Firebase AI exposes `model.generateContentStream(...)` / `chat.sendMessageStream(...)` returning a Kotlin `Flow<…>` — surface it as a `Flow<String>` of cumulative text, the way iOS used `AsyncThrowingStream`.
...
6. **Composable** — `feature/lounge/LoungeScreen.kt` (stateful wrapper + stateless content), themed with `JetCard`/`JetTheme`.
